      ...
      <form id="checkoutForm" class="grid lg:grid-cols-3 gap-8">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden scroll-reveal opacity-0 translate-y-8">
          ...
          <div class="p-6 space-y-3">
            {{-- Virtual Account --}}
            <label
              class="flex items-center gap-4 p-4 border-2 border-gray-200 rounded-xl cursor-pointer hover:border-gray-900 hover:bg-gray-50 transition group">
              <input type="radio" name="metode_pembayaran" value="va" class="w-5 h-5 text-gray-900"
                onchange="toggleChannelOptions()">
              <div class="flex-1">
                <p class="font-semibold text-gray-900">Virtual Account</p>
                <p class="text-sm text-gray-600">Transfer via Virtual Account</p>
              </div>
            ...
            <label
              class="flex items-center gap-4 p-4 border-2 border-gray-200 rounded-xl cursor-pointer hover:border-gray-900 hover:bg-gray-50 transition group">
              <input type="radio" name="metode_pembayaran" value="retail" class="w-5 h-5 text-gray-900"
                onchange="toggleChannelOptions()">
              <div class="flex-1">
                <p class="font-semibold text-gray-900">Retail Outlet</p>
                <p class="text-sm text-gray-600">Alfamart, Indomaret, Alfamidi</p>
              </div>
              <svg class="w-6 h-6 text-gray-400 group-hover:text-gray-900" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
  ...
  @push('scripts')
    <script>
      ...
      function toggleChannelOptions() {
        const paymentMethodElement = document.querySelector('input[name="metode_pembayaran"]:checked');
      ...
      document.getElementById('checkoutForm').addEventListener('submit', function (e) {
        ...
        const paymentMethodEl = document.querySelector('input[name="metode_pembayaran"]:checked');
        if (!paymentMethodEl) {
        ...
        if ((paymentMethod === 'va' || paymentMethod === 'ewallet' || paymentMethod === 'retail') && !channel) {
          alert('Silakan pilih channel pembayaran (Bank/E-Wallet/Retail)');
          return;
        }
